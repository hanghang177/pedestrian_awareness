...
def min_filter(v):
    ...
    q.put(v)
    q_list = list(q.queue)
    return np.min(q_list)

def callback(com_im):
    np_arr = np.fromstring(com_im.data, np.uint8)
    cv_image = cv2.imdecode(np_arr, cv2.IMREAD_COLOR)

    frame = cv2.rotate(cv_image, cv2.ROTATE_180)
    ...
    if head_size != 0:
        p_aware = min_filter(p_aware)
        if p_aware > 0.8:
            esc_val = 10.0
            cv2.putText(draw_frame, "GO", (0, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 3)
        else:
            esc_val = 0.0
            cv2.putText(draw_frame, "STOP", (0, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 3)

    pub_esc.publish(esc_val)
    pub_servo.publish(Float64(servo_ang))
...
try:
    ...
    # Open writer on cv2
    if output_video:
        writer = cv2.VideoWriter('test.mp4', cv2.VideoWriter_fourcc('M','P','4','V'), 15, (1280, 720))
    ...
    # Create Subscriber
    image_sub = rospy.Subscriber("/camera/image/compressed", CompressedImage, callback, queue_size=1)

    pub_esc = rospy.Publisher('esc', Float64, queue_size=10)
    pub_servo = rospy.Publisher('servo', Float64, queue_size=10)
